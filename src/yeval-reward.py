...
def get_armo(cfg, dataset):
    ...
    for item in tqdm(dataset, desc="Processing dataset"):
        ...
        refined_prompt = item["refined_instruction"]
        for j in range(len(item["refined_responses"])):
            generated_response = item["refined_responses"][j]
            score = rm([{"role": "user", "content": refined_prompt}, {"role": "assistant", "content": generated_response}])
            refined_similarity_scores.append(score)

        item['refined_similarity_scores'] = refined_similarity_scores
        item['avg_refined_similarity_score'] = sum(refined_similarity_scores) / len(refined_similarity_scores) if refined_similarity_scores else 0
        avg_refined_similarity_scores += sum(refined_similarity_scores)
